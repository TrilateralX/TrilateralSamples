...
var kitGL_glWeb_Ply = function(width_,height_) {
	...
	var inPosName = this.vertexPosition;
	var inColName = this.vertexColor;
	var isDynamic = true;
	if(isDynamic == null) {
		isDynamic = false;
	}
...
kitGL_glWeb_Sheet.prototype = {
	create: function(width_,height_,autoChild) {
		...
		this.gl = js_html__$CanvasElement_CanvasUtil.getContextWebGL(this.canvasGL,null);
	...
	,addQuads: function(clockWise,width_) {
		...
		if(clockWise && !this.lastClock) {
			if(this.count == 1) {
				...
				var cx = this.ncx;
				var cy = this.ncy;
				var windAdjust = _this.paintType.triangle(ax,ay,0,bx,by,0,cx,cy,0);
...
function trilateral3_drawing_Fill_triangulate(pen,sketch,fillForm) {
	...
	case 0:
		...
		while(_g < (triples.length / 3 | 0)) {
			...
			var cx = vert[c];
			var cy = vert[c + 1];
			var windAdjust = pen.paintType.triangle(ax,ay,0,bx,by,0,cx,cy,0);
			if(trilateral3_Trilateral.transformMatrix != null) {
		...
		while(_g < _g1) {
			...
			if(p[i].length != 0) {
				...
				while(_g6 < (triples.length / 3 | 0)) {
					...
					var cx1 = poly[c];
					var cy1 = poly[c + 1];
					var windAdjust = pen.paintType.triangle(ax1,ay1,0,bx1,by1,0,cx1,cy1,0);
					if(trilateral3_Trilateral.transformMatrix != null) {
						pen.paintType.transform(trilateral3_Trilateral.transformMatrix);
					}
					if(pen.useTexture) {
...
trilateral3_drawing_Sketch.prototype = {
	...
	,baseLine: function(x_,y_) {
		...
		var cx = exPrev_;
		var cy = eyPrev_;
		var windAdjust = _this1.paintType.triangle(ax,ay,0,bx,by,0,cx,cy,0);
		if(trilateral3_Trilateral.transformMatrix != null) {
